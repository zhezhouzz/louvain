...
module BaseGraph = struct
  ...
  let length (graph : graph) : int = Hashtbl.length graph

  let make_node_context (mat : weight option array array) :
      (node, node_context) Hashtbl.t =
    ...
    let rec init_f len idx =
      if len = idx then ()
        ...
        let _ =
          Hashtbl.add ctxs idx {self= idx; edages= Hashtbl.create length}
        in
        init_f len (idx + 1)
    in
          ...
          let _ =
            Array.fold_left
              (fun j ow ->
                ...
                | None -> j + 1
                | Some w ->
                    let _ = Hashtbl.add (Hashtbl.find ctxs i).edages j w in
                    j + 1 )
              0 vec
          in
          i + 1 )
  ...
  let get_edage (g : graph) (n_from : node) (n_goto : node) : edage =
    { from= n_from
    ; goto= n_goto
    ; weight= Hashtbl.find (Hashtbl.find g n_from).edages n_goto }

  let get_weight_opt (g : graph) (n_from : node) (n_goto : node) :
      weight option =
    Hashtbl.find_opt (Hashtbl.find g n_from).edages n_goto

  let fold_neighbors (f : 'a -> node -> 'a) (start : 'a) (g : graph) (n : node)
      : 'a =
          ...
          (fun neighbor wt2n r ->
            ...
            | None ->
                ...
                let _ =
                  ...
                  if Hashtbl.mem dest_ctx neighbor then ()
                  else Hashtbl.add dest_ctx neighbor wt2n
                in
